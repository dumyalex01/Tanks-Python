...
#update map text
def tank_boom(bullet_x,bullet_y,last_x,last_y,tank_number,x_tank,y_tank,a=5,b=20): #parametrii a si b sunt folositi pentru a diferentia 
	if tank_number==1:
		if last_x<=x_tank-a and last_y<=y_tank and bullet_x>=x_tank and bullet_y>=y_tank:
			return True
	if tank_number==2:
		if last_x>=x_tank+b and last_y<=y_tank and bullet_x<=x_tank and bullet_y>=y_tank:
			return True
	return False
...
def update_angle1(angle1):
	if angle1==0:
		text_angle1=font.render("Angle:0",True,(255,0,0),(15,135,255))
...
def update_angle2(angle2):
	if angle2==0:
		text_angle2=font.render("Angle:0",True,(255,0,0),(15,135,255))
...
def showmap(angle_1,speed_1,angle_2,speed_2,is_explosion1,is_explosion2,small_proj1,small_proj2):
       #desenare mapa
	window.fill((0,0,0))
	...
	for i in range(len(green_pixels_ylist)):
		if green_pixels_xlist[i]!=0 and green_pixels_ylist[i]!=0:
			pygame.draw.rect(window,(15,135,255),pygame.Rect(green_pixels_xlist[i],green_pixels_ylist[i],45,45))
...
def collision(bullet_x,bullet_y):
	...
	if(color==(0,250,0,255)):
		return True
	return False
def tank_collision(bullet_x,bullet_y,tank_x,tank_y):  #coliziune simpla cu tank-ul(pe toata lungimea si inaltimea sa)
	k=-20
	ok1=ok2=0
	while k<=25:
		if int(bullet_x)==tank_x+k:
			ok1=1   
		k=k+1
	
	k=-1
	while k<=10:
		if int(bullet_y)==tank_y+k:
			ok2=1
		k=k+1
	if ok1*ok2==1:
		return True
	return False
def tank_collision_littleprojectile(bullet_x,bullet_y,tank_x,tank_y):
	k=-5
	ok1=ok2=0
	while k<=20:
		if int(bullet_x)==tank_x+k:    #coliziunea cu un proiectil care da damage mare se realizeaza doar pe "plafonul" tankului
			ok1=1
		k=k+1
	k=-10
	while k<=10:
...
while running:
	...
	for event in pygame.event.get():
		...
		if event.type==pygame.KEYDOWN:
			...
			if event.key==pygame.K_2:
				if can_shoot() and number_of_special1>0:
					small_proj1=True
					gravity = 9.81
					velocity = speed1
					vx = velocity * math.cos(math.radians(angle1))
			...
			if event.key==pygame.K_u:
				if can_shoot() and number_of_special2>0:
					small_proj2=True
					gravity = 9.81
					velocity = -speed2
					vx = velocity * math.cos(math.radians(angle2))
	...
	if bullet_x1>=970:  #iesire din mapa +verificare
	...
	if bullet_x2<=9:  # verificare iesire din mapa, valoarea 9 a fost aleasa pentru a ajuta la atingerea tank-ului cand se foloseste puterea speciala
			bullet_x2=bullet_x2-1000  #iesire din mapa pentru a disparea proiectilul
			spacePressed=False
			special2=False
			small_proj2=False
	...
	if special1:
		 #atunci cand este activata puterea speciala 1, proiectilul "va cadea" peste tank sau se va deplaza orizontal catre acesta 
	...
	if special2:
		# atunci cand este activata puterea speciala a tankului 2, proiectilul "va cadea" peste tank sau se va deplasa orizontal catre el cand ajunge in apropiere
		if bullet_x2>=width//2:
			...
			if abs(bullet_y2-y_tank1)<=40:
			 ...
			 #iesire din mapa
	is_explosion1=False  #variabile care verifica daca este explozie, folosite pentru afisarea imaginii conforme exploziei
	is_explosion2=False
#verificarea coliziunii cu tank-urile
	if tank_collision(bullet_x1,bullet_y1,x_tank2,y_tank2)==True and small_proj1==False or (tank_boom(bullet_x1,bullet_y1,last_x,last_y,1,x_tank2,y_tank2)==True and small_proj1==False):
	...
	if tank_collision(bullet_x2,bullet_y2,x_tank1,y_tank1)==True and small_proj2==False or (tank_boom(bullet_x2,bullet_y2,last_x,last_y,2,x_tank1,y_tank1)==True and small_proj2==False):
		pygame.mixer.Sound.play(crash_sound)
		pygame.mixer.music.stop()
		if special2==True:
			life1=life1-10
 ...
 # verificarea coliziunii atunci cand se trimit proiectile cu damage mare
	if small_proj1==True:
		if tank_collision_littleprojectile(bullet_x1,bullet_y1,x_tank2,y_tank2):
			pygame.mixer.Sound.play(crash_sound)
			pygame.mixer.music.stop()
			life2=life2-40
			bullet_x1=bullet_x1+1000
	...
	if small_proj2==True:
		if tank_collision_littleprojectile(bullet_x2,bullet_y2,x_tank1,y_tank1):
			pygame.mixer.Sound.play(crash_sound)
			pygame.mixer.music.stop()
			life1=life1-40
			bullet_x2=bullet_x2-1000
			is_explosion1=True
			small_proj2=False
